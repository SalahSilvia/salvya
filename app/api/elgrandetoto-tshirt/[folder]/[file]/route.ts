...
import { isSafeArtistFolderOrFileName, resolveArtistCatalogTshirtFilePath } from "@/lib/artist-folder-catalog";
...
export async function GET(_request: Request, ctx: { params: Promise<{ folder: string; file: string }> }) {
  ...
  const filePath = resolveArtistCatalogTshirtFilePath("elgrandetoto", folder, file);
