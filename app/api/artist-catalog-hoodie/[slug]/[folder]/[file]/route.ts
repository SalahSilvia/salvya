...
import {
  ...
  resolveArtistCatalogHoodieFilePath,
...
export async function GET(_request: Request, ctx: { params: Promise<{ slug: string; folder: string; file: string }> }) {
  const { slug: rawSlug, folder: rawFolder, file: rawFile } = await ctx.params;
  const slug = rawSlug;
  const folder = decodeURIComponent(rawFolder);
  const file = decodeURIComponent(rawFile);

  if (!isArtistFolderCatalogSlug(slug) || !isSafeArtistFolderOrFileName(folder) || !isSafeArtistFolderOrFileName(file)) {
    return new Response("Not found", { status: 404 });
  }

  const filePath = resolveArtistCatalogHoodieFilePath(slug, folder, file);
