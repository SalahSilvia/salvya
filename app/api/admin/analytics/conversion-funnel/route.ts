...
import { computeConversionFunnel } from "@/lib/admin/analytics-snapshot";
...
export async function GET(request: NextRequest) {
  ...
  try {
    const data = await computeConversionFunnel(ctx.service, days);
    return rbacApiJsonWithAuthCookies(ctx.authResponse, { ok: true, data });
  } catch (e) {
    return rbacApiJsonWithAuthCookies(
      ctx.authResponse,
      { ok: false, error: e instanceof Error ? e.message : "funnel_failed" },
      { status: 500 },
    );
  }
}
