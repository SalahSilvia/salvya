...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!artist || artist.statusTag === "COMING SOON") {
    return buildPageMetadata({
      ...
      path: `/artist/${slug}/tshirt/${itemSlug}`,
      locale,
    });
  }

  const product = await loadPublishedPdpProduct(slug, itemSlug, "tshirt");
  if (!product) {
    return buildPageMetadata({
...
export default async function ArtistTshirtItemPage({ params }: Props) {
  const { locale, slug, itemSlug } = await params;
  const artist = await getStorefrontArtistBySlug(slug);
  ...
  const suggestedItems = await getSuggestedProductsForPdp(slug, { kind: "tshirt", itemSlug });
  ...
  return (
    <>
        ...
        data={[
          ...
          breadcrumbJsonLd(
            [
              ...
              { name: product.title, path: `/artist/${slug}/tshirt/${itemSlug}` },
            ],
            locale,
          ),
        ]}
      ...
      <ProductDbPdp
        ...
        checkoutHref={`/artist/${slug}/tshirt/${itemSlug}/checkout`}
