...
import { resolveTshirtCheckoutProduct } from "@/lib/catalog/resolve-checkout-product";
...
export async function generateMetadata({ params, searchParams }: Props): Promise<Metadata> {
  ...
  if (!artist || artist.statusTag === "COMING SOON") return { title: "Checkout — Salvya" };

  const recap = parsePreviewSelection(sp);
  const product = await resolveTshirtCheckoutProduct(slug, itemSlug, recap);
  if (product) {
    return { title: `Checkout · ${product.displayTitle} — Salvya` };
  }

  return { title: "Checkout — Salvya" };
}

export default async function TshirtItemCheckoutPage({ params, searchParams }: Props) {
  const { slug, itemSlug } = await params;
  const sp = await searchParams;
  const artist = await getStorefrontArtistBySlug(slug);
  ...
  const returnHref = `/artist/${slug}/tshirt/${itemSlug}`;
