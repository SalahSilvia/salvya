...
import { likedItemInputFromArtistFolderHoodie } from "@/lib/member/likes-from-card";
import { formatOversizeHoodieTitle, HOODIE_PRICE_LABEL } from "@/lib/shop-data";

/** ~2.5 tiles visible (two full + half peek) */
const VISIBLE_FRACTION = 2.5;
const GAP_PX = 12;

type Props = {
  items: ElgrandetotoFolderHoodieItem[];
  artistSlug?: string;
};

export function ElgrandetotoHoodieCarousel({ items, artistSlug = "elgrandetoto" }: Props) {
  ...
  const scrollByDir = useCallback((dir: -1 | 1) => {
    const el = scrollerRef.current;
    if (!el) return;
    const card = el.querySelector<HTMLElement>("[data-hoodie-card]");
  ...
  return (
    <div className="relative">
      ...
      <button
        ...
        aria-label="Scroll hoodies left"
      ...
      <button
        ...
        aria-label="Scroll hoodies right"
      ...
      <div
        ...
        tabIndex={0}
        aria-label="Hoodie carousel"
      >
        {items.map((item) => {
          const thumb = item.orderedFiles[0];
          const href = `/artist/${artistSlug}/item/${encodeURIComponent(item.folder)}`;
          return (
            <div
              key={item.folder}
              data-hoodie-card
              ...
              <Link href={href} className="block">
                <article className="group flex h-full flex-col rounded-2xl border border-white/[0.07] bg-gradient-to-b from-white/[0.05] via-[#0a0a0f] to-[#06060a] p-2 shadow-[0_18px_40px_-28px_rgba(0,0,0,0.9)] ring-1 ring-inset ring-white/[0.04] transition-[border-color,box-shadow,transform] duration-200 hover:border-[#2D6BFF]/28 hover:shadow-[0_22px_48px_-24px_rgba(45,107,255,0.35)] active:scale-[0.98]">
                  <div className="relative aspect-square w-full overflow-hidden rounded-xl bg-[#0c0c12] ring-1 ring-inset ring-white/[0.06]">
                    {thumb ? (
                      <img
                        src={artistCatalogHoodieImageSrc(artistSlug, item.folder, thumb)}
                        alt=""
                    ...
                    />
                    {thumb ? (
                      <ProductHeartButton input={likedItemInputFromArtistFolderHoodie(artistSlug, item, thumb)} />
                    ) : null}
                  </div>
                  <div className="mt-2.5 flex min-h-0 flex-1 flex-col gap-2 px-0.5 pb-0.5 pt-0.5">
                    <h3 className="line-clamp-3 min-h-0 text-[12px] font-semibold leading-snug tracking-[-0.015em] text-white sm:text-[13px]">
                      {formatOversizeHoodieTitle(item.title)}
                    </h3>
                    <div className="mt-auto flex items-center justify-between gap-2">
                      ...
                      <span className="text-[10px] font-semibold uppercase tracking-normal text-white/28">Hoodie</span>
                    </div>
                  </div>
                </article>
              </Link>
